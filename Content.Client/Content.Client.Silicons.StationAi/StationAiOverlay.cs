...
public sealed class StationAiOverlay : Overlay
{
	...
	{
		IoCManager.InjectDependencies<StationAiOverlay>(this);
	}
	...
	protected override void Draw(in OverlayDrawArgs args)
	{
		...
		if (!val.HasValue || val.GetValueOrDefault() != size)
		{
			...
			_stencilTexture = _clyde.CreateRenderTarget(args.Viewport.Size, new RenderTargetFormatParameters((RenderTargetColorFormat)1, false), (TextureSampleParameters?)null, "station-ai-stencil");
			_staticTexture = _clyde.CreateRenderTarget(args.Viewport.Size, new RenderTargetFormatParameters((RenderTargetColorFormat)1, false), (TextureSampleParameters?)null, "station-ai-static");
		}
		DrawingHandleWorld worldHandle = ((OverlayDrawArgs)(ref args)).WorldHandle;
		Box2Rotated worldBounds = args.WorldBounds;
		EntityUid? localEntity = ((ISharedPlayerManager)_player).LocalEntity;
		TransformComponent val2 = default(TransformComponent);
		_entManager.TryGetComponent<TransformComponent>(localEntity, ref val2);
		...
		_entManager.TryGetComponent<MapGridComponent>(val3, ref grid);
		BroadphaseComponent val4 = default(BroadphaseComponent);
		_entManager.TryGetComponent<BroadphaseComponent>(val3, ref val4);
		...
		if (grid != null && val4 != null)
		{
			...
			SharedTransformSystem obj = _entManager.System<SharedTransformSystem>();
			if (_accumulator <= 0f)
			{
				_accumulator = MathF.Max(0f, _accumulator + _updateRate);
				_visibleTiles.Clear();
				_entManager.System<StationAiVisionSystem>().GetView(Entity<BroadphaseComponent, MapGridComponent>.op_Implicit((val3, val4, grid)), worldBounds, _visibleTiles);
			}
			...
			((DrawingHandleBase)worldHandle).RenderInRenderTarget((IRenderTarget)(object)_staticTexture, (Action)delegate
			{
				...
				ShaderInstance val5 = _proto.Index<ShaderPrototype>(CameraStaticShader).Instance();
				((DrawingHandleBase)worldHandle).UseShader(val5);
		...
		((DrawingHandleBase)worldHandle).UseShader(_proto.Index<ShaderPrototype>(StencilDrawShader).Instance());
