...
// Type: Robust.Shared.ContentPack.IWritableDirProvider
...
public interface IWritableDirProvider
{
  ...
  (IEnumerable<ResPath> files, IEnumerable<ResPath> directories) Find(
    string pattern,
    bool recursive = true);
  ...
  {
    return this.Open(path, fileMode, fileMode == FileMode.Append ? FileAccess.Write : FileAccess.ReadWrite, FileShare.None);
  }
