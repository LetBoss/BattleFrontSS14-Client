...
// Type: Robust.Shared.Toolshed.TypeParsers.Math.InvalidNumber`1
...
public record InvalidNumber<T>(string Value) : IConError
{
  ...
  {
    return FormattedMessage.FromUnformatted($"The value {this.Value} is not a valid {typeof (T).PrettyName()}.");
  }
