...
// Type: Robust.Shared.Toolshed.Commands.Math.SubtractCommand
...
public sealed class SubtractCommand : ToolshedCommand
{
  ...
  public T Operation<T>([PipedArgument] T x, T y) where T : ISubtractionOperators<T, T, T> => x - y;

  [CommandImplementation(null)]
  [TakesPipedTypeAsGeneric]
  public IEnumerable<T> Operation<T>([PipedArgument] IEnumerable<T> x, IEnumerable<T> y) where T : ISubtractionOperators<T, T, T>
  {
