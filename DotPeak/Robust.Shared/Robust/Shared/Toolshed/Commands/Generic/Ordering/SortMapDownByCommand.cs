...
// Type: Robust.Shared.Toolshed.Commands.Generic.Ordering.SortMapDownByCommand
...
public sealed class SortMapDownByCommand : ToolshedCommand
{
  ...
  public override Type[] TypeParameterParsers => SortMapDownByCommand._parsers;
  ...
  {
    return (IEnumerable<TOrd>) input.Select<T, TOrd>((Func<T, TOrd>) (x => orderer.Invoke(x, ctx))).OrderDescending<TOrd>();
  }
}
