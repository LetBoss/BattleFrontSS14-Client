...
// Type: Robust.Shared.Serialization.TypeSerializers.Implementations.Primitive.IntSerializer
  ...
  BaseSerializerInterfaces.ITypeInterface<int>
{
  ...
  {
    return !Parse.TryInt32(node.Value.AsSpan(), out int _) ? (ValidationNode) new ErrorNode((DataNode) node, "Failed parsing int value: " + node.Value) : (ValidationNode) new ValidatedValueNode((DataNode) node);
  }
