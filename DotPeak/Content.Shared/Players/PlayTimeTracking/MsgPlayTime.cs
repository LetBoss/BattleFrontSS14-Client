...
// Type: Content.Shared.Players.PlayTimeTracking.MsgPlayTime
...
public sealed class MsgPlayTime : NetMessage
{
  ...
  public override void ReadFromBuffer(NetIncomingMessage buffer, IRobustSerializer serializer)
  {
    ...
    this.Trackers.EnsureCapacity(capacity);
    for (int index = 0; index < capacity; ++index)
      this.Trackers.Add(((NetBuffer) buffer).ReadString(), buffer.ReadTimeSpan());
  }

  public override void WriteToBuffer(NetOutgoingMessage buffer, IRobustSerializer serializer)
  {
    ((NetBuffer) buffer).WriteVariableInt32(this.Trackers.Count);
    foreach ((string key, TimeSpan timeSpan) in this.Trackers)
    {
      ((NetBuffer) buffer).Write(key);
      buffer.Write(timeSpan);
    }
  }
}
