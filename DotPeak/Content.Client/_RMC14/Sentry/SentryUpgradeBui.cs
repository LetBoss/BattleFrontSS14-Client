...
// Type: Content.Client._RMC14.Sentry.SentryUpgradeBui
...
public sealed class SentryUpgradeBui : BoundUserInterface
{
  ...
  {
    base.Open();
    this._menu = BoundUserInterfaceExt.CreateWindow<SentryUpgradeMenu>((BoundUserInterface) this);
    SentryComponent sentryComponent;
    if (this.EntMan.TryGetComponent<SentryComponent>(this.Owner, ref sentryComponent))
    {
      ...
      if (upgrades != null)
      {
        foreach (EntProtoId entProtoId in upgrades)
        {
          ...
          if (this._prototypes.TryIndex(upgradeId, ref entityPrototype))
          {
            ...
            ((BaseButton) menuTextureButton2).OnButtonDown += (Action<BaseButton.ButtonEventArgs>) (_ => this.SendPredictedMessage((BoundUserInterfaceMessage) new SentryUpgradeBuiMsg(upgradeId)));
