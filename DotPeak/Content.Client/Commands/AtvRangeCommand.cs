// Decompiled with JetBrains decompiler
// Type: Content.Client.Commands.AtvRangeCommand
// Assembly: Content.Client, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: B4469588-B038-4783-B6EC-1EFF6592A364
// Assembly location: C:\Users\sus\Desktop\SS14_VFS_Dump_20260624_230444\Content.Client.dll

using Content.Client.Atmos.EntitySystems;
using Robust.Shared.Console;
using Robust.Shared.GameObjects;
using Robust.Shared.IoC;

#nullable enable
namespace Content.Client.Commands;

internal sealed class AtvRangeCommand : LocalizedCommands
{
  [Dependency]
  private IEntitySystemManager _entitySystemManager;

  public virtual string Command => "atvrange";

  public virtual string Help
  {
    get
    {
      return this.LocalizationManager.GetString($"cmd-{base.Command}-help", ("command", (object) base.Command));
    }
  }

  public virtual void Execute(IConsoleShell shell, string argStr, string[] args)
  {
    if (args.Length != 2)
    {
      shell.WriteLine(base.Help);
    }
    else
    {
      float result1;
      if (!float.TryParse(args[0], out result1))
      {
        shell.WriteError(this.LocalizationManager.GetString($"cmd-{base.Command}-error-start"));
      }
      else
      {
        float result2;
        if (!float.TryParse(args[1], out result2))
          shell.WriteError(this.LocalizationManager.GetString($"cmd-{base.Command}-error-end"));
        else if ((double) result1 == (double) result2)
        {
          shell.WriteError(this.LocalizationManager.GetString($"cmd-{base.Command}-error-zero"));
        }
        else
        {
          AtmosDebugOverlaySystem entitySystem = this._entitySystemManager.GetEntitySystem<AtmosDebugOverlaySystem>();
          entitySystem.CfgBase = result1;
          entitySystem.CfgScale = result2 - result1;
        }
      }
    }
  }
}
