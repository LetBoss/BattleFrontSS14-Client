...
public sealed class RobustILGenerator
{
	...
	{
		List<string> list = _locals.Select<(Type, int), string>(((Type, int) e) => $"LOCAL: {e.Item1} at {e.Item2}").ToList();
		list.Add("===== CODE =====");
		list.AddRange(_log.Select<(object, object), string>(((object, object) e) => $"{e.Item1} - {e.Item2}"));
		return list.ToArray();
	}
	...
	{
		LocalBuilder localBuilder = _generator.DeclareLocal(localType, pinned);
		_locals.Add((localType, localBuilder.LocalIndex));
		return localBuilder;
	}
