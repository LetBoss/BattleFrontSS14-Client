...
[Serializable]
[NetSerializable]
public sealed class TotalJobsTimeRequirement : JobRequirement, ISerializationGenerated<TotalJobsTimeRequirement>, ISerializationGenerated
{
	...
	public bool TryRequirementsMet(IReadOnlyDictionary<string, TimeSpan> playTimes, out FormattedMessage? reason, IEntityManager entManager, IPrototypeManager prototypes)
	{
		...
		JobGroupComponent comp = default(JobGroupComponent);
		if (!prototypes.Index(Group).TryGetComponent<JobGroupComponent>(ref comp, entManager.ComponentFactory))
		{
			Logger.GetSawmill("job.requirements").Error($"No {"DepartmentGroupComponent"} found on entity {Group}");
			return true;
		}
		foreach (ProtoId<JobPrototype> jobId in comp.Jobs)
		{
			string proto = prototypes.Index<JobPrototype>(jobId).PlayTimeTracker;
			trackers.Add(proto);
		}
	...
	public void InternalCopy(ref TotalJobsTimeRequirement target, ISerializationManager serialization, SerializationHookContext hookCtx, ISerializationContext? context = null)
	{
		...
		target = (TotalJobsTimeRequirement)definitionCast;
		if (!serialization.TryCustomCopy<TotalJobsTimeRequirement>(this, ref target, hookCtx, false, context))
		{
	...
	[Obsolete("Use ISerializationManager.CopyTo instead")]
	public void Copy(ref TotalJobsTimeRequirement target, ISerializationManager serialization, SerializationHookContext hookCtx, ISerializationContext? context = null)
	{
	...
	{
		TotalJobsTimeRequirement cast = (TotalJobsTimeRequirement)target;
	...
	public override TotalJobsTimeRequirement Instantiate()
	{
		return new TotalJobsTimeRequirement();
	}
}
