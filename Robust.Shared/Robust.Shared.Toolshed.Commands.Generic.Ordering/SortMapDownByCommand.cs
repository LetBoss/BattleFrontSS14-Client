...
public sealed class SortMapDownByCommand : ToolshedCommand
{
	...
	public IEnumerable<TOrd> SortBy<TOrd, T>(IInvocationContext ctx, [PipedArgument] IEnumerable<T> input, Block<T, TOrd> orderer) where TOrd : IComparable<TOrd>
	{
		return input.Select((T x) => orderer.Invoke(x, ctx)).OrderDescending();
	}
}
